...
class DefaultVibeMatchmaker : VibeMatchmaker {
    /**
    ...
    override fun compareBeacon(myBeacon: Beacon, discoveredBeacon: Beacon): MatchmakingResult {
        val vibeSim = calculateCosineSimilarity(myBeacon.vibeVector, discoveredBeacon.vibeVector)
        val likesSim: Float
        var dislikesModifier = 0f
        val baseScore: Float
        val overallScore: Float
        if (myBeacon.profile != null && discoveredBeacon.profile != null) {
            likesSim = calculateCosineSimilarity(
                myBeacon.profile!!.likesVector,
                discoveredBeacon.profile!!.likesVector
            )
            ...
            if (discoveredBeaconDislikes != null) {
                dislikesModifier -= (DEALBREAKER_PENALTY * calculateCosineSimilarity(
                    myBeacon.profile!!.likesVector,
                    discoveredBeaconDislikes
                ))
            }
            ...
            if (myBeaconDislikes != null && discoveredBeaconDislikes != null) {
                dislikesModifier += (HATER_BONUS * calculateCosineSimilarity(
                    myBeaconDislikes,
                    discoveredBeaconDislikes
                ))
            }
            baseScore = (vibeSim * VIBE_WEIGHT) + (likesSim * LIKES_WEIGHT)
            overallScore = (baseScore + dislikesModifier).coerceIn(0f, 1f)
        } else {
            likesSim = 0f
            baseScore = vibeSim
